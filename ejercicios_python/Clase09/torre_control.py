...
class TorreDeControl:
    ...
    def asignar_pista(self):
        try:
            #los arribos tienen prioridad de pista
            if len(self.arribos.items):
                vuelo = self.arribos.desencolar()
                return f'El vuelo {vuelo} despegó con exito.'
            else:
                vuelo = self.despegues.desencolar()
                return f'El vuelo {vuelo} aterrizó con exito.'
    ...
    def ver_estado(self):
        print(f'Vuelos esperando para aterrizar: {", ".join(self.arribos.items)}\n'
              f'Vuelos esperando para despegar: {", ".join(self.despegues.items)}')
